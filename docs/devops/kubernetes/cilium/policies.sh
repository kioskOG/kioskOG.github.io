# Replace the master & kube-node2 as per your node names.
...
kubectl exec -it $Node2_CILIUM_POD -n kube-system -- cilium endpoint list
    kubectl exec -it $Node2_CILIUM_POD -n kube-system -- cilium endpoint get  878  # here 878 is the Endpoint id.
